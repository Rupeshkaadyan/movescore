import type { Metadata } from "next";
import Link from "next/link";
import { ArrowRight } from "lucide-react";
import { Container, SectionHeading } from "@/components/ui/primitives";
import { CITIES, cityLabel } from "@/lib/data/cities";
import { buildMetadata } from "@/lib/seo";
import { money, percent } from "@/lib/format";

export const metadata: Metadata = buildMetadata({
  title: "U.S. city profiles — cost of living, housing, jobs and taxes",
  description:
    "Full profiles for 30 major U.S. cities: median rent, home prices, wages, taxes, commute, climate, schools, safety and neighbourhood data.",
  path: "/cities",
});

export default function CitiesIndexPage() {
  const sorted = [...CITIES].sort((a, b) => b.metrics.population - a.metrics.population);

  return (
    <Container className="py-10">
      <SectionHeading
        eyebrow="Cities"
        title="Thirty major U.S. cities, modelled properly"
        description="We started with 30 cities instead of 10,000 on purpose. Each profile carries the same full set of sourced metrics, so comparisons are like-for-like."
      />

      <div className="mt-8 overflow-x-auto">
        <table className="w-full min-w-[760px] text-sm">
          <thead>
            <tr className="border-b border-line text-left">
              <th scope="col" className="py-3 pr-4 font-semibold">City</th>
              <th scope="col" className="py-3 pr-4 text-right font-semibold">Cost index</th>
              <th scope="col" className="py-3 pr-4 text-right font-semibold">Median 1-bed rent</th>
              <th scope="col" className="py-3 pr-4 text-right font-semibold">Median home</th>
              <th scope="col" className="py-3 pr-4 text-right font-semibold">Wage index</th>
              <th scope="col" className="py-3 pr-4 text-right font-semibold">Unemployment</th>
              <th scope="col" className="py-3 text-right font-semibold">Profile</th>
            </tr>
          </thead>
          <tbody>
            {sorted.map((city) => (
              <tr key={city.slug} className="border-b border-line/70">
                <th scope="row" className="py-3 pr-4 text-left font-medium text-ink">
                  {cityLabel(city)}
                </th>
                <td className="py-3 pr-4 text-right tabular-nums">{city.metrics.colIndex}</td>
                <td className="py-3 pr-4 text-right tabular-nums">{money(city.metrics.medianRent1br)}</td>
                <td className="py-3 pr-4 text-right tabular-nums">{money(city.metrics.medianHomePrice)}</td>
                <td className="py-3 pr-4 text-right tabular-nums">{city.metrics.salaryIndex}</td>
                <td className="py-3 pr-4 text-right tabular-nums">{percent(city.metrics.unemploymentRate, 1)}</td>
                <td className="py-3 text-right">
                  <Link
                    href={`/cities/${city.slug}`}
                    className="inline-flex items-center gap-1 font-medium text-brand hover:underline"
                  >
                    Open
                    <ArrowRight className="h-3.5 w-3.5" aria-hidden />
                  </Link>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </Container>
  );
}
